...
但是注意把类的对象和成员函数的定义写在CPP文件而非头文件（虽然这样做在语法上没有问题），但因为函数只能被定义一次。若放在头文件中，当头文件被多个CPP文件include时，函数会被多次定义。

当你定义了一个类叫test，定义它的一个对象时请不要用如下方式初始化

```c++
test myclass();//编译器会把它看作函数声明
//所以应该下面的方式
//test myclass(1，1，1);//法一
//test myclass;//法二
//test myclass = test(1，1，1);//法三
```
...
2.构造函数和析构函数的调用一般是一一对应的，换言之，整个程序中，构造函数（包括复制，普通，默认）和析构函数的调用的次数是相等的。
...
若对重载运算符同时作为成员函数和友元函数，可能会引发二义性错误，导致编译错误，详情看P322。

## 坑：类对象作为函数参数以值传递

值传递时产生类的副本。若原来对象中有指针，并用new动态分配了内存。函数返回时，会把副本中指针指向的内存释放，此时原对象中的指针变为野指针。

## 三类继承的特点

参考：https://blog.csdn.net/chlele0105/article/details/22413157

公有继承(public) ：基类的公有成员和保护成员作为派生类的成员时，它们都保持原有的状态，而基类的私有成员仍然是私有的，不能被这个派生类的子类所访问。

私有继承(private) ：基类的公有成员和保护成员都作为派生类的私有成员，并且不能被这个派生类的子类所访问。
...
编译器处理虚函数方式：（虚函数表）给每个对象添加一个隐藏成员，隐藏成员中保存了一个指向函数地址数组的指针。

虚函数必须有定义

构造函数不可以是虚函数，但析构函数可以是虚函数，且析构函数为虚函数在对基类和派生类对象进行内存释放时很合适。

虚函数可以有多个版本，而在使用基类或派生类的指针或引用时会发生动态绑定，来确定调用哪个版本的虚函数，详见C++ Primer第五版P537

派生类和基类的虚函数形参类型必须一致，返回类型一般也要一致，当然在返回类型为类本身的指针或引用时，可以不同。
